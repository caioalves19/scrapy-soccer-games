...
serie_a = [f'https://www.cbf.com.br/amp/futebol-brasileiro/competicoes/campeonato-brasileiro-serie-a/2021/00{i+1}' for i in range (10)]
...
class CbfGamesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        local_jogo = tratar_local(response.css('.col-xs-12 span::text').getall())    
        ...
        nome_campeonato = response.url.split('/')[-3].replace('-', ' ').replace('serie', 'série').title()
        
        jogo.add_css('time_mandante', '.jogo-equipe-nome-completo::text', lambda v: v[0])
