...
for campeonato in campeonatos:
    for k, v in campeonato.items():
        links.append(f"https://www.flashscore.com.br/futebol/{k}/{v[0]}/calendario")
...
class FlashscoresGamesSpider(scrapy.Spider):
    ...
    def __init__(self):
        f = open(
            "D:\Caio\Projetos-Python\scrapy-soccer-games\\futebol_interior\\flashscore_games.json",
            "w",
        ).close()
        self.html_lista = []
        ...
        for link in links:
            ...
            try:
                botao_popup = driver.find_element(By.ID, "onetrust-accept-btn-handler")
                botao_popup.click()
            except:
                pass
            ...
            while True:
                sleep(2)
                try:
                    botao_mais_jogos = driver.find_element(By.CLASS_NAME, "event__more")
    ...
    def parse(self, response):
        for i in range(len(self.html_lista)):
            ...
            jogos_site = resp.xpath(
                "//div[@class='sportName soccer']//div[@class='event__round event__round--static' or @class='event__time' or contains(@class, 'event__homeParticipant') or contains(@class, 'event__awayParticipant')]//text()"
            ).getall()
        ...
        for i in range(len(self.jogos_detalhados)):
            ...
            data, hora = (
                resp.css(".duelParticipant__startTime div::text")
                .get()
                .replace(".", "/")
                .split(" ")
            )
            ...
            time_mandante, estado = nome_time_libertadores(times[0].split(" (")[0])
            time_visitante, nada = nome_time_libertadores(times[1].split(" (")[0])

            rodada = resp.css(".tournamentHeader__country a::text").get().strip()
